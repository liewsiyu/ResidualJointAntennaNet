...
# 数据预处理
dataset = pd.read_csv(open(r'D:\天线选择 郭志斌\正确数据集\101022\全信道矩阵(p=50行列).csv')).iloc[:, 1:]
...
for i in range(0, length):
    print(i)
    subCapacity, i1_Capacity, i2_Capacity, j1_Capacity, j2_Capacity = JTRAS_Capacity_Algorithm_101022(dataset[i],p)  # 第一个返回的子信道容量
    subGain, i1_Gain, i2_Gain, j1_Gain, j2_Gain = JTRAS_Gain_Algorithm_101022(dataset[i])  # 第一个返回的子信道增益
...
subCapacityData.to_csv(r'D:\天线选择 郭志斌\正确数据集\101022\JTRAS子信道容量(p=50行列).csv')
capacityLabelData.to_csv(r'D:\天线选择 郭志斌\正确数据集\101022\JTRAS容量标签(p=50行列).csv')
subGainData.to_csv(r'D:\天线选择 郭志斌\正确数据集\101022\JTRAS子信道增益(p=50行列).csv')
gainLabelData.to_csv(r'D:\天线选择 郭志斌\正确数据集\101022\JTRAS增益标签(p=50行列).csv')
