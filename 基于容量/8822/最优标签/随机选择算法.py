...
a = 50
m = n = 8
dataset = pd.read_csv(open(r'D:\天线选择 郭志斌\正确数据集\全信道矩阵(p=50行列).csv')).iloc[:200000, 1:]
label_array = pd.read_csv(open(r'D:\天线选择 郭志斌\正确数据集\容量标签(p=50行列).csv')).iloc[:200000, 1:]
...
fullChainCapacity = pd.read_csv(r"D:\天线选择 郭志斌\正确数据集\全信道容量(p=50行列).csv").iloc[:, 1:]
fullChainCapacity= fullChainCapacity[0:200000]
fullChainCapacity = np.asarray(fullChainCapacity, np.float32)

subChainCapacity = pd.read_csv(r"D:\天线选择 郭志斌\正确数据集\子信道容量(p=50行列).csv").iloc[:, 1:]
